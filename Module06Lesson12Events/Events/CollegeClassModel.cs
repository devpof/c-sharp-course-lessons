    ...
    public class CollegeClassModel
    {
        ...
        public string SignUpStudent(string studentName)
        {
            ...
            if (enrolledStudents.Count < MaximumStudents)
            {
                ...
                // fire off the event
                // question mark means if no one is listening, we do not want to throw an exception
                // question mark means if this is not null, do this next thing, otherwise ignore.
                if (enrolledStudents.Count == MaximumStudents)
                {
